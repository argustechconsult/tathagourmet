...
const Calculator: React.FC = () => {
  ...
  const handleSendWhatsApp = () => {
    const selectedServices = selections.services.map(id => SERVICES.find(s => s.id === id)?.name).join(', ');
    const selectedPromos = selections.promos.map(id => PROMOS.find(p => p.id === id)?.name).join(', ');
...
*Duração:* 4 horas`;
  ...
  return (
    <section id="calculator" className="py-20 bg-gray-900 text-white scroll-mt-24">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4 font-kids uppercase">Monte seu Evento</h2>
          <p className="text-gray-400">Selecione os itens desejados e veja o valor na hora!</p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {SERVICES.map(item => (
                  ...
                  >
                    ...
                    <div className="text-pink-400 text-xs mt-1">R$ {item.price.toFixed(2)}</div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <h3 className="text-xl font-bold mb-6 flex items-center gap-2 text-yellow-400 font-kids">
                <span className="w-8 h-8 bg-yellow-400 rounded-full flex items-center justify-center text-sm text-gray-900">2</span>
                Combos & Promoções
              </h3>
                ...
                {PROMOS.map(item => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="px-2 py-0.5 bg-yellow-400 text-gray-900 text-[10px] font-bold rounded uppercase">Promoção</div>
                    </div>
                    <div className="font-bold">{item.name}</div>
                    <div className="text-xs text-gray-400 mt-1 mb-3">Includes: {item.items.join(', ')}</div>
                    <div className="text-yellow-400 font-bold">R$ {item.price.toFixed(2)}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white text-gray-900 rounded-3xl p-8 sticky top-24 shadow-2xl">
              <h3 className="text-2xl font-bold mb-6 font-kids uppercase">Resumo</h3>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Data do Evento</label>
                  <input 
                    type="date" 
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 focus:ring-2 focus:ring-pink-500 outline-none"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Localização</label>
                  <input 
                    type="text" 
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl p-3 focus:ring-2 focus:ring-pink-500 outline-none" 
              ...
              <div className="border-t border-gray-100 pt-6 space-y-3">
                <div className="flex justify-between text-gray-500">
                  ...
                  <span className="font-bold">{selections.services.length + selections.promos.length}</span>
                </div>
                ...
                <p className="text-[10px] text-gray-400 text-center">*Valor referente a 4 horas de serviço.</p>
              </div>
